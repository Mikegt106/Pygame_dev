...
class Player:
    def __init__(self, x: int, y: int, config: dict):
        ...
        self.block = BlockSystem(
            block_chance=0.60,
            fail_stun=0.40,
            cooldown=1.0,         # ✅ cooldown aanpassen doe je hier
            shield_scale=0.1,
            pushback_force=300.0, # ✅ pushback aanpassen doe je hier
        )
        ...
        self.block_push_damping = 1200.0  # hoger = sneller stoppen (feel)
    ...
    # --- mana passthroughs (main.py blijft werken) ---
    @property
    def mana(self):
    ...
    def update(self, keys, dt: float, projectiles: list, projectile_cfg: dict):
        ...
        # start jump (niet tijdens protect/attack/hurt/dead)
        if jump_pressed and self.on_ground and (self.anim.state != "attack") and (not self.block.blocking) and (self.hurt_timer <= 0) and (not self.dead):
        ...
        # ✅ APPLY BLOCK PUSHBACK (smooth)
        if self.block_push_vel != 0.0:
            self.pos.x += self.block_push_vel * dt

            # exponential-ish damping towards 0
        ...
        # movement is blocked during protect/attack/hurt
        ...
        if can_move:
            ...
            # als je niet kan bewegen, mana regen “rust” (optioneel)
    ...
    def take_damage(self, amount: int):
        ...
        # block roll ONLY when hit
        result, push_force = self.block.try_block()

        if result == BlockResult.SUCCESS:
            # ✅ NO DAMAGE, NO HURT, keep protect animation
            direction = -1 if self.facing_right else 1  # duw weg van enemy richting
            self.block_push_vel = direction * push_force
            return

        if result == BlockResult.FAIL:
            # ❌ failed block: stun (cooldown zit al in block system)
            self.hurt_timer = max(self.hurt_timer, self.block.get_fail_stun())

        # damage (normal hit or failed block)
        self.hp -= amount
        if self.hp < 0:
            self.hp = 0
    ...
    def draw(self, screen: pygame.Surface):
        ...
        # shield overlay (alleen op hit via timers)
