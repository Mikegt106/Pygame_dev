...
class Player:
    ...
    def take_damage(self, amount: int) -> bool:
        ...
        if result == BlockResult.SUCCESS:
            # ✅ parry: geen damage, geen hurt
            # pushback: achteruit t.o.v. facing
            direction = 1 if self.facing_right else -1
            self.block_push_vel = -direction * self.block.pushback_force
            return True

        if result == BlockResult.FAIL:
            # ❌ failed block: stun player (en cooldown zit al in BlockSystem)
            self.hurt_timer = max(self.hurt_timer, self.block.fail_stun)
            # hier wil je WEL damage krijgen -> val door naar damage
